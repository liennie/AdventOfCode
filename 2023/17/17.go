...
func main() {
	...
	// Part 1
	_, l, err := path.Shortest(path.GraphFunc[Node](func(n Node) []path.Edge[Node] {
		...
		for _, dir := range []space.Point{{X: 1}, {Y: -1}, {X: -1}, {Y: 1}} {
			...
			for i := 1; i <= 3; i++ {
				tdir := dir.Scale(i)
				npos := n.pos.Add(tdir)
				if !aabb.Contains(npos) {
					break
				}

				tfrom := n.from.Add(tdir)
				if ints.Abs(tfrom.X) > 3 || ints.Abs(tfrom.Y) > 3 {
					break
				}

				cost += blocks[npos.Y][npos.X]
		...
		return edges
	}), Node{pos: space.Point{X: 0, Y: 0}}, path.EndFunc[Node](func(n Node) bool { return n.pos == end }))
